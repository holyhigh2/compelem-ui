...
import { PortalWidget } from '@/components/datadisplay/portal/PortalWidget'

// @tag 默认延迟注册，需显式调用 defineComponents 完成 customElements.define
defineComponents()

const TAG = 'ce-portal-widget'

describe('ce-portal-widget', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(PortalWidget)
  })
