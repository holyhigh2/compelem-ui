...
import { AccordionItem } from '@/components/nav/accordion/AccordionItem'

// @tag 默认延迟注册，需显式调用 defineComponents 完成 customElements.define
defineComponents()

const TAG = 'ce-accordion-item'

describe('ce-accordion-item', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(AccordionItem)
  })
