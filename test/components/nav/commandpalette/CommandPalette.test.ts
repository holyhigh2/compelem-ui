...
import { CommandPalette } from '@/components/nav/commandpalette/CommandPalette'

// @tag 默认延迟注册，需显式调用 defineComponents 完成 customElements.define
defineComponents()

const TAG = 'ce-command-palette'

describe('ce-command-palette', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(CommandPalette)
  })
