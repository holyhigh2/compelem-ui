...
import { ListGroup } from '@/components/layout/list/ListGroup'

// @tag 默认延迟注册，需显式调用 defineComponents 完成 customElements.define
defineComponents()

const TAG = 'ce-list-group'
// 最小必要数据：缺少时组件初始化会中断
const FIXTURE = { attrs: { 'title': 't' } }

describe('ce-list-group', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(ListGroup)
  })
