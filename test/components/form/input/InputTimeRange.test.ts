...
import { InputTimeRange } from '@/components/form/input/InputTimeRange'

// @tag 默认延迟注册，需显式调用 defineComponents 完成 customElements.define
defineComponents()

const TAG = 'ce-input-time-range'

describe('ce-input-time-range', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(InputTimeRange)
  })
