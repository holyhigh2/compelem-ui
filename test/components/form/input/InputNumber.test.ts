...
const TAG = 'ce-input-number'

describe('ce-input-number', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(InputNumber)
  })
  ...
  it('默认属性值', async () => {
    ...
    expect((el as any).scrollable).toBe(true)
    expect((el as any).control).toBe(false)
  })
