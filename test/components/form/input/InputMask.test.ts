...
const TAG = 'ce-input-mask'
// 最小必要数据：缺少时组件初始化会中断
const FIXTURE = { attrs: { 'mask': '00:00' } }

describe('ce-input-mask', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(InputMask)
  })
  ...
  it('默认属性值', async () => {
    ...
    expect((el as any).placeholderChar).toBe('_')
    expect((el as any).guide).toBe(false)
    expect((el as any).greedy).toBe(false)
  })
