...
import { InputNumberRange } from '@/components/form/input/InputNumberRange'

// @tag 默认延迟注册，需显式调用 defineComponents 完成 customElements.define
defineComponents()

const TAG = 'ce-input-number-range'

describe('ce-input-number-range', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(InputNumberRange)
  })
