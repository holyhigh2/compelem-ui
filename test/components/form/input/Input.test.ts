...
describe('ce-input', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(Input)
  })
  ...
  it('默认属性值', async () => {
    ...
    expect((el as any).showPassword).toBe(false)
    expect((el as any).inside).toBe(false)
    expect((el as any).autoselect).toBe(false)
  })
