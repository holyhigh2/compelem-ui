...
import { TimerPicker } from '@/components/picker/timepicker/TimePicker'

// @tag 默认延迟注册，需显式调用 defineComponents 完成 customElements.define
defineComponents()

const TAG = 'ce-timer-picker'

describe('ce-timer-picker', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(TimerPicker)
  })
  ...
  it('默认属性值', async () => {
    ...
    expect((el as any).hideSeconds).toBe(false)
  })
