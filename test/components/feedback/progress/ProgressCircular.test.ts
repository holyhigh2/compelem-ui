...
import { ProgressCircular } from '@/components/feedback/progress/ProgressCircular'

// @tag 默认延迟注册，需显式调用 defineComponents 完成 customElements.define
defineComponents()

const TAG = 'ce-progress-circular'

describe('ce-progress-circular', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(ProgressCircular)
  })
  ...
  it('默认属性值', async () => {
    const el = await mount(TAG)
    expect((el as any).width).toBe(5)
    expect((el as any).r).toBe(16)
    expect((el as any).color).toBe('')
    expect((el as any).indeterminate).toBe(false)
    expect((el as any).rounded).toBe(true)
    expect((el as any).hideTrack).toBe(false)
  })
