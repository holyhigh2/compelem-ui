...
const TAG = 'ce-notification'
// 最小必要数据：缺少时组件初始化会中断
const FIXTURE = { attrs: { 'descr': 'test' } }

describe('ce-notification', () => {
  it('注册为自定义元素', () => {
    expect(customElements.get(TAG)).toBe(Notification)
  })
  ...
  it('属性响应：bordered', async () => {
    const el = await mount(TAG, { attrs: { ...FIXTURE.attrs, 'bordered': true } })
