...
ToastContainer.className = "l-toast-container";
ToastContainer.innerHTML = `
<div class="--con --top-left"></div>
<div class="--con --top-center"></div>
<div class="--con --top-right"></div>
<div class="--con --bottom-left"></div>
<div class="--con --bottom-center"></div>
<div class="--con --bottom-right"></div>
`;
const PostionMap: Record<ToastPostion, HTMLElement> = {
  [ToastPostion.TopLeft]: ToastContainer.querySelector<HTMLElement>('.--top-left')!,
  [ToastPostion.TopCenter]: ToastContainer.querySelector<HTMLElement>('.--top-center')!,
  [ToastPostion.TopRight]: ToastContainer.querySelector<HTMLElement>('.--top-right')!,
  [ToastPostion.BottomLeft]: ToastContainer.querySelector<HTMLElement>('.--bottom-left')!,
  [ToastPostion.BottomCenter]: ToastContainer.querySelector<HTMLElement>('.--bottom-center')!,
  [ToastPostion.BottomRight]: ToastContainer.querySelector<HTMLElement>('.--bottom-right')!,
}
...
class Toast implements IToast {
  ...
  clear(): void {
    each(ToastContainer.querySelectorAll<HTMLElement>('l-message'), msgNode => {
      if (!msgNode) return;
      msgNode.classList.remove(ClassName)
      msgNode.addEventListener('transitionend', (e: Event) => {
        let t = e.target as HTMLElement;
